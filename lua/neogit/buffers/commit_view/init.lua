...
local function diff_visit_file(self, component, worktree)
  ...
  if worktree then
    ...
    if not target_commit then
      notification.warn("Unable to retrive parent commit")
      return nil, cursor
    end
...
local function is_jumpable_hunk_line_component(c)
  return c.options.line_hl == "NeogitDiffContext" or c.options.line_hl == "NeogitDiffAdd" or
    c.options.line_hl == "NeogitDiffDelete"
end
