...
describe("char_diff_spans", function()
  it("returns empty spans for identical strings", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("hello", "hello")
    eq({}, old_spans)
    eq({}, new_spans)
    eq(0, distance)
  end)
  ...
  it("returns empty spans for two empty strings", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("", "")
    eq({}, old_spans)
    eq({}, new_spans)
    eq(0, distance)
  end)
  ...
  it("handles completely different strings", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("abc", "xyz")
    eq({ { 0, 3 } }, old_spans)
    eq({ { 0, 3 } }, new_spans)
    eq(1.0, distance)
  end)
  ...
  it("detects a word change with shared prefix and suffix", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("d.iteritems()", "d.items()")
  ...
  it("detects an insertion in the middle", function()
    local old_spans, new_spans, distance =
      diff.char_diff_spans("range(0, options):", "range(0, int(options)):")
  ...
  it("detects word replacement in a sentence", function()
    local old_spans, new_spans, distance =
      diff.char_diff_spans("safe to read the commit number from", "safe to read build info from")
  ...
  it("detects appended text", function()
    local old_spans, new_spans, distance =
      diff.char_diff_spans("self.table[index] =", "self.table[index] = candidates")
    eq({}, old_spans)
  ...
  it("reports high distance for completely unrelated lines", function()
    local _, _, distance = diff.char_diff_spans(
  ...
  it("handles deletion from one side only", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("abcdef", "abef")
    -- "cd" removed at positions 2..4
    eq({ { 2, 4 } }, old_spans)
    eq({}, new_spans)
    assert.is_true(distance > 0 and distance < 1)
  end)

  it("handles insertion to one side only", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("abef", "abcdef")
    eq({}, old_spans)
  ...
  it("detects a single character change", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("aaa", "aba")
    eq({ { 1, 2 } }, old_spans)
    eq({ { 1, 2 } }, new_spans)
    assert.is_true(distance > 0 and distance < 0.5)
  end)

  -- Inspired by delta: comma moves outside bracket
  it("detects moved punctuation", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("[element,]", "[element],")
    assert.is_true(#old_spans > 0 or #new_spans > 0)
    assert.is_true(distance < 0.5)
  end)

  it("handles one empty string", function()
    local old_spans, new_spans, distance = diff.char_diff_spans("hello", "")
    eq({ { 0, 5 } }, old_spans)
    eq({}, new_spans)
    eq(1.0, distance)
  end)
